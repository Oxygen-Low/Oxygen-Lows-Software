...
router.post("/", authenticateRepoRequest, apiLimiter, async (req, res) => {
  ...
  try {
    ...
    const { storagePath, size } = await repoManager.createRepo(
      repo.id,
      user.id,
      name,
      token,
    ...
    const { error: updateError } = await supabase
      .from("repositories")
      .update({ storage_path: storagePath, zip_size_bytes: size })
      .eq("id", repo.id);
    if (updateError)
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      const supabase = getSupabaseClient(token);
      const { data: newRepoId, error } = await supabase.rpc("fork_repository", {
        p_repo_id: id,
      });
      if (error) return res.status(500).json({ error: error.message });
      if (!validateId(newRepoId))
        return res
          .status(500)
          .json({ error: "Invalid new repo ID from server" });
      ...
      const originalPath = await repoManager.ensureLoaded(
        id,
        originalRepo.storage_path,
        token,
      );
      ...
      const { size } = await repoManager.uploadToStorage(
        newRepoId,
        storagePath,
        token,
      );
  ...
  async (req, res) => {
    ...
    if (branch === "main" && repo.owner_id !== user.id) {
      return res
        ...
        .json({
          error:
            "Only the repository owner can push to the main branch. Please use a fork and Pull Request.",
        });
    }
    ...
    try {
      ...
      const tempDir = path.resolve(
        ...
        `${id}-edit-${crypto.randomBytes(4).toString("hex")}`,
      );
      try {
        ...
        await tempGit.commit(message || "Web edit");
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from("repository_issues")
        .insert({ repo_id: id, title, body, author_id: user.id })
        .select()
        .single();
      if (error) return res.status(500).json({ error: error.message });
      res.json(data);
...
router.post(
  ...
  async (req, res) => {
    ...
    if (
      !SAFE_REF_REGEX.test(source_branch) ||
      !SAFE_REF_REGEX.test(target_branch)
    ) {
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const diff = await simpleGit(repoManager.getRepoPath(id)).raw([
        "diff",
        "--",
        pr.target_branch,
        pr.source_branch,
    ...
    try {
      ...
      if (
        ...
        return res
          ...
          .json({
            error:
              "Only the repository owner or admin can merge to the main branch.",
          });
      }
      ...
      const tempDir = path.resolve(
        path.dirname(repoPath),
        `${id}-merge-${crypto.randomBytes(8).toString("hex")}`,
      );
      try {
        await fs.ensureDir(tempDir);
        await simpleGit().clone(repoPath, tempDir);
