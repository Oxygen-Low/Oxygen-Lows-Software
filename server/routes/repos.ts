...
router.post("/", authenticateRepoRequest, async (req, res) => {
  ...
  try {
    const { storagePath, size } = await repoManager.createRepo(repoId, user.id, name);
    if (initReadme) {
...
router.get("/:id/tree", authenticateRepoRequest, authorizeRepoAccess, async (req, res) => {
  const { id } = req.params;
  const { ref = "HEAD", path: treePath = "" } = req.query;
  try {
...
router.put("/:id/file", authenticateRepoRequest, authorizeRepoAccess, async (req, res) => {
  ...
  try {
    ...
    await fs.ensureDir(path.dirname(path.join(tempDir, filePath)));
