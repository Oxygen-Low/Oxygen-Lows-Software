export const ko = {
  ...
  security: {
    ...
    chatbotChats: "챗봇 대화 및 AI 공급자 키",
    chatbotChatsDesc: "AI 대화, 메시지 기록, 시스템 프롬프트 및 사용자 정의 AI 공급자 API 키를 암호화합니다.",
    ...
    architectureTitle: "영지식 및 개인정보 보호 아키텍처",
    arch1Title: "클라이언트 측 AES-256 암호화",
    arch1Desc: "모든 암호화는 데이터를 전송하거나 저장하기 전에 브라우저 내에서 직접 수행됩니다. 서버 및 데이터베이스 관리자도 내용을 읽을 수 없습니다.",
    arch2Title: "사용자 지정 AI 공급자 API 키",
    arch2Desc: "향후 지원될 사용자 지정 AI 공급자 키(OpenAI, Anthropic, OpenRouter 등) 및 대화가 마스터 키로 암호화되어 자격 증명이 안전하게 유지됩니다.",
    arch3Title: "마스터 키 백업",
    arch3Desc: "Oxygen Low는 영지식 암호화를 사용하므로 마스터 키를 분실하면 데이터를 복구할 수 없습니다. 키를 안전한 비밀번호 관리자에 보관하세요.",
  },
} as const;
