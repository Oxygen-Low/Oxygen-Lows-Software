...
import { ShieldCheck, AlertTriangle, Check, ShieldAlert } from "lucide-react";
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-950 p-4">
        <Card className="w-full max-w-md bg-slate-900 border-red-900/50">
          <CardHeader>
            ...
            <CardTitle className="text-center text-white">Authorization Error</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <Card className="w-full max-w-md bg-slate-900 border-slate-800 shadow-2xl">
        <CardHeader className="text-center">
          ...
          <CardDescription className="text-slate-400 mt-2">
            This application would like to connect to your <strong>Oxygen Low's Software Accounts</strong>.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
              ...
              {scopes && scopes.length > 0 ? (
                scopes.map((scope: string) => (
                  <li key={scope} className="flex items-start gap-3 text-sm text-slate-400">
                    ...
                    <span>
                      {scope === "openid" && "Access your unique user identifier"}
                      {scope === "email" && "View your email address"}
                      {scope === "profile" && "Access your profile information (name, picture)"}
          ...
          <div className="flex items-center gap-2 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
            ...
            <p className="text-xs text-slate-400">
              Only authorize applications you trust. This app will be able to access the data listed above.
            </p>
          </div>
        </CardContent>
