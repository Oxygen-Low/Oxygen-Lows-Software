...
export default function Account() {
  ...
  const fetchAccountData = useCallback(async () => {
    ...
    if (pic) { if (pic.image_path) { supabase.storage.from("Storage").createSignedUrl(pic.image_path, 3600).then(({ data }) => { if (data) setProfilePicture({ ...pic, image_url: data.signedUrl }); else setProfilePicture(pic); }); } else { setProfilePicture(pic); } }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      await supabase.from("profile_pictures").upsert({ user_id: session.user.id, image_url: stableUrl, crop_data: croppedArea });
