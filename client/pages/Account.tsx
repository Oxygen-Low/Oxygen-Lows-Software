...
export default function Account() {
  ...
  const fetchAccountData = useCallback(async () => {
    ...
    if (pic) {
      if (pic.image_path) {
        supabase.storage
          .from("Storage")
          .createSignedUrl(pic.image_path, 3600)
          .then(({ data }) => {
            if (data) setProfilePicture({ ...pic, image_url: data.signedUrl });
            else setProfilePicture({ ...pic, image_url: "" });
          })
          .catch(() => setProfilePicture({ ...pic, image_url: "" }));
      } else {
    ...
    if (ints) {
      ...
      if (settings.integrations && key) {
        const processedInts = await Promise.all(
          ints.map(async (i: any) => ({
            ...i,
            base_url: i.base_url
              ? await decrypt(i.base_url, key).catch(() => "[Encrypted]")
              : undefined,
          })),
        );
    ...
    try {
      ...
      if (target === "integrations") {
        if (!isEnabling) {
          if (
            !window.confirm(
              "Disabling encryption for integrations will PERMANENTLY DELETE all your saved API keys and Base URLs. Please make sure you have them saved elsewhere. Continue?",
            )
          ) {
      ...
      toast({
        title: "Success",
        description: `${target} ${isEnabling ? "encrypted" : "decrypted"} successfully.`,
      });
    } catch (e: any) {
      toast({
  ...
  const handleResetEncryption = async () => {
    if (
      !window.confirm(
        "Are you sure you want to reset encryption? This will PERMANENTLY DELETE all currently encrypted data. Unencrypted data will not be affected.",
      )
    ) {
      return;
    }
    ...
    try {
      ...
      toast({
        ...
        description:
          "All encrypted data has been deleted and encryption settings have been reset.",
      });
    } catch (e: any) {
      toast({ title: "Error", description: e.message, variant: "destructive" });
    } finally {
          ...
          <TabsContent value="profile" className="space-y-6">
            <Card className="bg-slate-900/50 border-slate-800">
              <CardHeader>
                ...
                <CardDescription>
                  How others see you on Oxygen Low.
                </CardDescription>
              </CardHeader>
                    ...
                    <StorageFileSelector
                      ...
                      trigger={
                        <button
                          ...
                          className="absolute -bottom-2 -right-2 p-2 bg-cyan-600 hover:bg-cyan-700 text-white rounded-xl shadow-lg"
                        >
            ...
            <Card className="bg-slate-900/50 border-slate-800">
              <CardContent className="pt-6 space-y-4">
                {PROVIDERS.map((p) => (
                  <div
                    key={p.id}
                    className="p-4 rounded-xl bg-slate-950/50 border border-slate-800/50 space-y-4"
                  >
                    ...
                    <div className="flex gap-3">
                      {p.hasUrl && (
                        <Input
                          ...
                          value={
                            baseUrlInputs[p.id] ??
                            integrations.find((i) => i.provider === p.id)
                              ?.base_url ??
                            ""
                          }
                          onChange={(e) =>
            ...
            <Card className="bg-slate-900/50 border-slate-800">
              <CardContent className="pt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                {["github", "gitlab", "google", "discord"].map((p) => (
                  <Button
                    key={p}
                    variant={isLinked(p) ? "secondary" : "outline"}
                    onClick={() => handleLinkIdentity(p)}
                    disabled={isLinked(p)}
                    className={`h-12 justify-start gap-3 ${isLinked(p) ? "bg-green-500/10 text-green-400" : "bg-slate-950"}`}
                  >
          ...
          <TabsContent value="security" className="space-y-6">
            <Card className="bg-slate-900/50 border-slate-800">
              <CardHeader>
                ...
                <CardDescription>
                  Protect your data with a masterkey. Data is encrypted in your
                  browser before being sent to the server.
                  <br />
                  <strong className="text-red-400">
                    Warning: If you lose your masterkey, your data cannot be
                    recovered.
                  </strong>
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                {!generatedKey &&
                  Object.values(encryptionSettings).every((v) => !v) && (
                    <div className="space-y-4">
                      <div className="space-y-2">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setSelectedKeyLength(Number(e.target.value))
                          }
                ...
                {generatedKey && (
                  <div className="p-4 rounded-xl bg-slate-950 border border-cyan-900/50 space-y-4">
                    <p className="text-xs font-mono break-all text-cyan-400">
                      {generatedKey}
                    </p>
                    <Button
                      variant="outline"
