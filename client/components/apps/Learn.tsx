...
const COURSES: Course[] = [
  {
    ...
    description: "Learn the fundamentals of how computers work, their components, and operating systems.",
    category: "Computing",
    subCategory: "Basics",
    ...
    sections: [
      ...
      },
      {
        ...
        content: "Computer hardware includes the physical parts of a computer. These are the things you can touch. To have a working computer, several key components must work together.",
        subsections: [
          {
            id: "cpu",
            title: "CPU (Central Processing Unit)",
            content: "The CPU is often called the 'brain' of the computer. It performs most of the processing inside the computer. It handles all instructions it receives from hardware and software running on the computer. Modern CPUs have multiple 'cores' allowing them to do many things at once.",
            model: "cpu"
          },
          {
            id: "ram",
            title: "RAM (Random Access Memory)",
            content: "RAM is your system's short-term memory. Whenever your computer performs calculations, it temporarily stores the data in the RAM until it is needed. This short-term memory disappears when the computer is turned off. More RAM allows more programs to be open simultaneously.",
            model: "ram"
          },
          {
            id: "motherboard",
            title: "Motherboard",
            content: "The motherboard is the computer's main circuit board. It's a thin plate that holds the CPU, memory, connectors for the hard drive and optical drives, expansion cards to control the video and audio, and connections to your computer's ports (such as USB ports).",
            model: "motherboard"
          },
          {
            id: "storage",
      ...
      },
      {
        ...
        content: "An operating system (OS) is the most important software that runs on a computer. It manages the computer's memory and processes, as well as all of its software and hardware. It also allows you to communicate with the computer without knowing how to speak the computer's language.",
        subsections: [
           {
             id: "os-types",
             title: "Common Operating Systems",
             content: "The three most common operating systems for personal computers are Microsoft Windows, macOS, and Linux. Modern operating systems use a graphical user interface (GUI) that lets you use your mouse to click icons, buttons, and menus."
           },
           {
             id: "os-role",
             title: "The OS as a Manager",
             content: "The OS acts like a traffic cop, ensuring that different programs don't interfere with each other and that hardware resources like the CPU and RAM are shared fairly among all running applications."
           }
        ]
      }
    ]
...
export function LearnApp() {
  ...
  const categories = useMemo(() => {
    ...
    COURSES.forEach(c => {
      ...
      if (!cats[c.category][c.subCategory]) cats[c.category][c.subCategory] = [];
          ...
          {Object.keys(categories).map(cat => (
            ...
            >
              <CardHeader>
                ...
                <CardDescription>Explore courses in {cat.toLowerCase()}</CardDescription>
              </CardHeader>
            </Card>
          ))}
          <Card
          ...
          <div className="flex items-center gap-2 text-sm overflow-x-auto pb-2">
            ...
            {isBrowsing && (
               <>
                 ...
                 <span className="text-slate-500 whitespace-nowrap">Library</span>
               </>
            )}
            ...
            {selectedCategory && (
              <>
                ...
                <Button variant="ghost" size="sm" onClick={() => {
                  setSelectedSubCategory(null);
                  setSelectedSubSubCategory(null);
                }} className="text-slate-400 whitespace-nowrap">{selectedCategory}</Button>
              </>
            )}
            {selectedSubCategory && (
              <>
                ...
                <Button variant="ghost" size="sm" onClick={() => {
                  setSelectedSubSubCategory(null);
                }} className="text-slate-400 whitespace-nowrap">{selectedSubCategory}</Button>
              </>
            )}
            {selectedSubSubCategory && (
              <>
                <ChevronRight className="w-4 h-4 text-slate-600 shrink-0" />
                <span className="text-cyan-400 px-3 whitespace-nowrap">{selectedSubSubCategory}</span>
              </>
            )}
          </div>
                ...
                >
                  ...
                  <CardHeader>
                    <div className="flex justify-between items-start mb-2">
                      <Badge variant="outline" className="text-xs border-slate-700 text-slate-400">{course.subSubCategory}</Badge>
                      {isCompleted && (
                ...
                {subSubs.map(ss => (
                  ...
                  >
                    <div className="text-left w-full flex justify-between items-center">
                      <div>
                        <div className="font-medium text-white group-hover:text-cyan-400 transition-colors">{ss}</div>
                        <div className="text-xs text-slate-500">View courses</div>
                      </div>
      ...
      <div className="animate-in fade-in slide-in-from-right-4 duration-500 h-full flex flex-col">
        <div className="flex items-center justify-between gap-4 mb-6 sticky top-0 bg-slate-950/80 backdrop-blur-md z-10 py-4 border-b border-slate-900">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-xl font-bold text-white leading-none mb-1">{selectedCourse.name}</h3>
              <p className="text-xs text-slate-500">{selectedCourse.category} / {selectedCourse.subCategory} / {selectedCourse.subSubCategory}</p>
            </div>
          </div>
          <div className="flex items-center gap-3 pr-4">
             ...
             <span className="text-xs font-mono text-slate-400 w-8 text-right">{Math.round(progress[selectedCourse.id] || 0)}%</span>
          </div>
        </div>
          ...
          <div className="max-w-3xl mx-auto space-y-12 pb-32 pt-8">
            {selectedCourse.sections.map(section => (
              <section key={section.id} className="space-y-6">
                <div className="space-y-2">
                  <h4 className="text-3xl font-bold text-white tracking-tight">{section.title}</h4>
                ...
                {section.subsections?.map(sub => (
                  <div key={sub.id} className="bg-slate-900/40 rounded-2xl p-8 border border-slate-800/50 space-y-6 hover:bg-slate-900/60 transition-colors">
                    <h5 className="text-2xl font-semibold text-white flex items-center gap-3">
                      {sub.model === 'cpu' && <Cpu className="w-6 h-6 text-cyan-500" />}
                      {sub.model === 'ram' && <Box className="w-6 h-6 text-cyan-500" />}
                      {sub.model === 'motherboard' && <Layers className="w-6 h-6 text-cyan-500" />}
                      {!sub.model && <div className="w-2 h-2 rounded-full bg-cyan-500" />}
                      {sub.title}
                    </h5>
            ...
            <div className="pt-12 border-t border-slate-900 flex flex-col items-center gap-6">
              ...
              <div className="text-center">
                <h5 className="text-2xl font-bold text-white mb-2">Congratulations!</h5>
                <p className="text-slate-400">You have completed the "{selectedCourse.name}" course.</p>
              </div>
              <Button
                size="lg"
