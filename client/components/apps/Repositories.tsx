...
function RepoDetail({ repo, onBack }: { repo: any, onBack: () => void }) {
  ...
  const username = profile?.username || session?.user?.user_metadata?.username || "username";
  const cloneUrl = `${window.location.protocol}//${username}@${window.location.host}/api/git/${repo.profiles?.username}/${repo.name}.git`;
