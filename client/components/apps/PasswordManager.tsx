...
function generatePassword(opts: GeneratorOptions): string {
  ...
  if (mode === "passphrase") {
    ...
    return Array.from(arr)
      .map((n) => BIP39_WORDS[n % BIP39_WORDS.length])
      .join("-");
  }
  let chars = "";
  if (uppercase) {
...
function OtpLiveDisplay({
  ...
  const [progress, setProgress] = useState<number>(() => getTotpProgress(30));
  ...
  const CB = ({
    ...
    >
      <input
        ...
        className="rounded border-slate-600 accent-cyan-500 w-3.5 h-3.5"
      />
      {label}
    </label>
  );
    ...
    <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-4 space-y-4">
      <div className="flex items-center gap-2 justify-between">
        <span className="text-xs font-semibold text-slate-300 flex items-center gap-1.5">
          ...
          {t("passwords.generator", undefined, "Password Generator")}
        </span>
        <div className="flex gap-1">
          {(["random", "passphrase", "pin"] as const).map((m) => (
            ...
            >
              {m === "random"
                ? t("passwords.genModeRandom", undefined, "Random")
                : m === "passphrase"
                  ? t("passwords.genModePassphrase", undefined, "Passphrase")
                  : t("passwords.genModePIN", undefined, "PIN")}
            </button>
          ))}
        </div>
      </div>
      <div className="flex items-center gap-2">
        <div className="flex-1 font-mono text-sm bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-slate-100 break-all min-h-[2.25rem] leading-relaxed select-all">
          {value || (
            <span className="text-slate-500 italic">No password generated</span>
          )}
        </div>
        <button
          type="button"
        ...
        <div className="space-y-1">
          <div className="flex justify-between items-center">
            <span className="text-[10px] text-slate-500">
              {t("passwords.strength", undefined, "Strength")}
            </span>
            <span
              className={cn(
      ...
      <div className="space-y-3">
        <div className="space-y-1">
          <div className="flex justify-between text-[10px] text-slate-500">
            <span>
              {t("passwords.length", undefined, "Length")}: {opts.length}
            </span>
          </div>
          <Slider
            min={opts.mode === "passphrase" ? 15 : opts.mode === "pin" ? 4 : 6}
            max={opts.mode === "passphrase" ? 50 : 64}
            step={1}
            value={[opts.length]}
        ...
        {opts.mode === "random" && (
          <div className="grid grid-cols-2 gap-2">
            <CB
              label={t("passwords.uppercase", undefined, "Uppercase (A-Z)")}
              checked={opts.uppercase}
              onChange={(v) => setOpts((o) => ({ ...o, uppercase: v }))}
            />
            <CB
              label={t("passwords.lowercase", undefined, "Lowercase (a-z)")}
              checked={opts.lowercase}
              onChange={(v) => setOpts((o) => ({ ...o, lowercase: v }))}
            />
            <CB
              label={t("passwords.numbers", undefined, "Numbers (0-9)")}
              checked={opts.numbers}
              onChange={(v) => setOpts((o) => ({ ...o, numbers: v }))}
            />
            <CB
              label={t("passwords.symbols", undefined, "Symbols (!@#...)")}
              checked={opts.symbols}
              onChange={(v) => setOpts((o) => ({ ...o, symbols: v }))}
            />
            <CB
              label={t(
                "passwords.avoidAmbiguous",
                undefined,
      ...
      {onApply && (
        ...
        >
          ...
          {t("passwords.applyPassword", undefined, "Apply to Password Field")}
        </Button>
      )}
    </div>
  );
    ...
    <div className="w-full max-w-2xl mx-auto my-8 px-4">
      <Card className="bg-slate-900/80 border border-amber-500/30 backdrop-blur-md shadow-2xl overflow-hidden relative">
        ...
        <CardHeader className="text-center space-y-3 pb-4 pt-6">
          ...
          <div className="space-y-1">
            <CardTitle className="text-xl sm:text-2xl font-bold text-white tracking-tight">
              {t(
                "passwords.encryptionRequiredTitle",
                undefined,
                "Encryption Required",
              )}
            </CardTitle>
            <CardDescription className="text-slate-400 text-xs sm:text-sm max-w-md mx-auto leading-relaxed">
              {t(
                "passwords.encryptionRequiredDesc",
                undefined,
        ...
        <CardContent className="space-y-4 pt-2 pb-6 px-6 sm:px-8 text-center">
          <Button
            onClick={onEnable}
            className="bg-gradient-to-r from-cyan-600 to-cyan-500 hover:from-cyan-500 hover:to-cyan-400 text-white font-semibold gap-2 shadow-lg shadow-cyan-950/50 px-6 py-2.5 h-auto"
          >
            <Lock className="w-4 h-4" />
            {t(
              "passwords.enableEncryptionButton",
              undefined,
...
export function PasswordManagerApp() {
  ...
  const [encryptionEnabled, setEncryptionEnabled] = useState(() =>
    isCategoryEncryptionEnabled("passwords"),
  );
  const [encryptionLocked, setEncryptionLocked] = useState(() =>
    isCategoryLocked("passwords"),
  );
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const decrypted = await Promise.all(
        (data || []).map((item) => decryptPasswordData(item, key)),
      );
      setPasswords(decrypted as PasswordRecord[]);
    } catch (err: any) {
      toast.error(
        err?.message ||
          t("passwords.fetchError", undefined, "Failed to load passwords"),
      );
    } finally {
      setFetching(false);
    }
  ...
  const handleEnableEncryption = useCallback(async () => {
    ...
    toast.success(
      t(
        "passwords.encryptionEnabledToast",
        undefined,
  ...
  const filtered = useMemo(() => {
    let list = passwords.filter((p) => {
      ...
      return (
        ...
        extractDomain(p.url).toLowerCase().includes(q)
      );
    });
    list = [...list].sort((a, b) => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!formPassword.trim()) {
      toast.error(
        t("passwords.passwordRequired", undefined, "Password is required"),
      );
      return;
    }
    if (!session?.user?.id) return;
    ...
    try {
      ...
      if (isCategoryEncryptionEnabled("passwords") && key)
        payload = await encryptPasswordData(payload, key);
      if (editingId) {
        ...
        if (error) throw error;
        toast.success(
          t("passwords.updatedToast", undefined, "Password updated"),
        );
      } else {
      ...
      toast.error(
        err?.message ||
          t("passwords.saveError", undefined, "Failed to save password"),
      );
    } finally {
      setSaving(false);
    }
  ...
  const handleEditInForm = (record: PasswordRecord) => {
    ...
    setShowGenerator(false);
    setGeneratorValue(record.password);
  ...
  const handleOpenEditDialog = (record: PasswordRecord) => {
    ...
    setShowEditGenerator(false);
    setEditGeneratorValue(record.password);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (error) throw error;
      toast.success(t("passwords.deletedToast", undefined, "Password deleted"));
      ...
      toast.error(
        err?.message ||
          t("passwords.deleteError", undefined, "Failed to delete password"),
      );
    } finally {
      setDeleting(false);
    }
  ...
  const handleCopyPassword = async (id: string, pw: string) => {
    ...
    setCopiedId(id);
    toast.success(
      t("passwords.copiedToast", undefined, "Password copied to clipboard"),
    );
  ...
  const SORT_OPTIONS: { key: SortKey; label: string }[] = [
    {
      key: "title_asc",
      label: t("passwords.sortTitleAZ", undefined, "Title (A-Z)"),
    },
    {
      key: "title_desc",
      label: t("passwords.sortTitleZA", undefined, "Title (Z-A)"),
    },
    { key: "url_asc", label: t("passwords.sortUrlAZ", undefined, "URL (A-Z)") },
    {
      key: "url_desc",
      label: t("passwords.sortUrlZA", undefined, "URL (Z-A)"),
    },
    {
      key: "updated_desc",
      label: t("passwords.sortUpdatedNewest", undefined, "Updated (Newest)"),
    },
    {
      key: "updated_asc",
      label: t("passwords.sortUpdatedOldest", undefined, "Updated (Oldest)"),
    },
    {
      key: "created_desc",
      label: t("passwords.sortCreatedNewest", undefined, "Created (Newest)"),
    },
    {
      key: "created_asc",
      label: t("passwords.sortCreatedOldest", undefined, "Created (Oldest)"),
    },
  ];

  if (!encryptionEnabled)
    return <EncryptionRequiredBanner onEnable={handleEnableEncryption} />;
  if (encryptionLocked)
    return (
      <EncryptionRequiredPrompt
        ...
        categoryLabel={t("passwords.appTitle", undefined, "Password Manager")}
      />
    );
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <CardTitle className="text-base text-white">
                  {editingId
                    ? t("passwords.editPassword", undefined, "Edit Password")
                    : t("passwords.addPassword", undefined, "Add Password")}
                </CardTitle>
            ...
            <form onSubmit={handleSave} className="space-y-3">
              <div className="space-y-1.5">
                <Label className="text-xs text-slate-400">
                  {t("passwords.titleLabel", undefined, "Title")}{" "}
                  <span className="text-slate-600">
                    ({t("common.optional", undefined, "Optional")})
                  </span>
                </Label>
                <Input
                  value={formTitle}
              ...
              <div className="space-y-1.5">
                <Label className="text-xs text-slate-400">
                  {t("passwords.urlLabel", undefined, "URL / Website")}{" "}
                  <span className="text-slate-600">
                    ({t("common.optional", undefined, "Optional")})
                  </span>
                </Label>
                <div className="flex gap-2">
                  <Input
                    ...
                    placeholder={t(
                      "passwords.urlPlaceholder",
                      undefined,
                      "https://example.com",
                    )}
                    className="bg-slate-950 border-slate-800 text-slate-100 text-sm flex-1"
                  />
                  {formUrl && (
                    <a
                      href={
                      ...
                      className="p-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-cyan-400 transition flex items-center"
                      title="Open URL"
                    >
              ...
              <div className="space-y-1.5">
                <Label className="text-xs text-slate-400">
                  {t("passwords.passwordLabel", undefined, "Password")}{" "}
                ...
                <div className="relative">
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      setGeneratorValue(e.target.value);
                    }}
                    placeholder={t(
                      "passwords.passwordPlaceholder",
                      undefined,
                      "Enter or generate a password...",
                    )}
                    className="bg-slate-950 border-slate-800 text-slate-100 text-sm pr-9 font-mono"
                    required
                  />
                  <button
                    type="button"
                    ...
                    <div className="flex-1 h-1 bg-slate-800 rounded-full overflow-hidden">
                      <div
                        className={cn(
                          ...
                          formPasswordStrength.color,
                        )}
                        style={{
                          width: `${Math.min(100, (formPasswordStrength.score / 7) * 100)}%`,
                        }}
                      />
                    </div>
              ...
              >
                ...
                {showGenerator
                  ? t("passwords.hideGenerator", undefined, "Hide Generator")
                  : t("passwords.showGenerator", undefined, "Show Generator")}
                {showGenerator ? (
              ...
              {showGenerator && (
                <PasswordGeneratorPanel
                  ...
                  onChange={(v) => setGeneratorValue(v)}
                  onApply={(v) => {
                    setFormPassword(v);
                    setGeneratorValue(v);
                  }}
              ...
              <div className="space-y-1.5">
                <Label className="text-xs text-slate-400">
                  {t(
                    "passwords.otpSecretLabel",
                    undefined,
                    "One-Time Password (OTP) Secret / 2FA Key",
                  )}{" "}
                  <span className="text-slate-600">
                    ({t("common.optional", undefined, "Optional")})
                  </span>
                ...
                <Input
                  ...
                  placeholder={t(
                    "passwords.otpSecretPlaceholder",
                    undefined,
                    "e.g. JBSWY3DPEHPK3PXP or otpauth://...",
                  )}
                  className="bg-slate-950 border-slate-800 text-slate-100 text-xs font-mono"
                />
                <p className="text-[11px] text-slate-500 leading-relaxed">
                  {t(
                    "passwords.otpSecretHelp",
                    undefined,
                ...
                {formOtpSecret.trim() &&
                  (validateTotpSecret(formOtpSecret) ? (
                    ...
                    <p className="text-[11px] text-rose-400">
                      {t(
                        "passwords.invalidOtpSecret",
                        undefined,
                        "Invalid OTP secret key",
                      )}
                    </p>
                  ))}
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs text-slate-400">
                  {t("passwords.notes", undefined, "Notes")}{" "}
                  <span className="text-slate-600">
                    ({t("common.optional", undefined, "Optional")})
                  </span>
                </Label>
                <Textarea
                  ...
                  placeholder={t(
                    "passwords.notesPlaceholder",
                    undefined,
                    "Additional info, security questions, etc.",
                  )}
                  className="bg-slate-950 border-slate-800 text-slate-100 text-sm resize-none min-h-[70px]"
                />
              </div>
              <div className="flex gap-2 pt-1">
                <Button
                  type="submit"
                  disabled={saving}
                  className="flex-1 bg-cyan-600 hover:bg-cyan-500 text-white font-medium gap-2"
                >
                  {saving ? (
                  ...
                  {editingId
                    ? t("passwords.updateButton", undefined, "Update Password")
                    : t("passwords.saveButton", undefined, "Save Password")}
                </Button>
                {editingId && (
                  <Button
                    type="button"
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
          <div className="relative flex-1">
            ...
            <Input
              ...
              className="pl-9 bg-slate-900 border-slate-800 text-slate-100 text-sm"
            />
          </div>
          <div className="flex items-center gap-2 shrink-0">
            ...
            >
              {passwords.length} {t("passwords.entries", undefined, "entries")}
            </Badge>
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="outline"
                  size="sm"
                  className="border-slate-700 text-slate-400 hover:text-white gap-1.5 h-9"
                >
                  <ArrowUpDown className="w-3.5 h-3.5" />
                  {t("passwords.sort", undefined, "Sort")}
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent
                align="end"
              ...
              >
                <DropdownMenuLabel className="text-xs text-slate-500">
                  {t("passwords.sortBy", undefined, "Sort by")}
                </DropdownMenuLabel>
                ...
                {SORT_OPTIONS.map((opt) => (
                  <DropdownMenuItem
                    ...
                    className={cn(
                      ...
                      sortKey === opt.key && "text-cyan-400 font-medium",
                    )}
                  >
                    {sortKey === opt.key && (
            ...
            <p className="text-slate-500 text-sm">
              {passwords.length === 0
                ? t(
                    ...
                    undefined,
                    "No passwords saved yet. Add one using the form on the left.",
                  )
                : t(
                    "passwords.noResults",
                    undefined,
            ...
            <div className="space-y-3 pr-2">
              {filtered.map((record) => {
                ...
                const displayTitle =
                  record.title ||
                  domain ||
                  t("passwords.untitled", undefined, "Untitled Password");
                const isRevealed = revealedIds[record.id];
                const isCopied = copiedId === record.id;
                return (
                  <div
                    key={record.id}
                    className="p-4 rounded-xl border border-slate-800 bg-slate-900/60 hover:border-slate-700 transition-all group"
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0 space-y-1.5">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <Badge
                            variant="outline"
                            className="bg-emerald-500/10 text-emerald-400 border-emerald-500/30 text-[10px] font-mono shrink-0"
                          >
                            AES-256-GCM
                          </Badge>
                          {record.otp_secret && (
                            <Badge
                              variant="outline"
                              className="bg-cyan-500/10 text-cyan-400 border-cyan-500/30 text-[10px] font-mono shrink-0 flex items-center gap-1"
                            >
                        ...
                        {record.url && (
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-xs text-cyan-400 hover:text-cyan-300 hover:underline truncate flex items-center gap-1 max-w-xs"
                          >
                            {domain}
                        ...
                        <div className="flex items-center gap-2">
                          <span className="font-mono text-xs text-slate-300 select-all truncate max-w-[200px] sm:max-w-[300px]">
                            {isRevealed
                              ? record.password
                              : "•".repeat(
                                  Math.min(20, record.password.length),
                                )}
                          </span>
                          <button
                            type="button"
                        ...
                        <p className="text-[10px] text-slate-600">
                          {t("passwords.updated", undefined, "Updated")}{" "}
                      ...
                      <div className="flex flex-col gap-1.5 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          type="button"
                          onClick={() =>
                            handleCopyPassword(record.id, record.password)
                          }
                        ...
                        <button
                          ...
                          title={t("passwords.edit", undefined, "Edit")}
                        >
                        ...
                        <button
                          ...
                          title={t("passwords.delete", undefined, "Delete")}
                        >
      ...
      <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        <DialogContent className="bg-slate-900 border-slate-800 max-w-lg">
          ...
          <div className="space-y-3 py-2">
              ...
              <div className="relative">
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    setEditGeneratorValue(e.target.value);
                  }}
                  className="bg-slate-950 border-slate-800 text-slate-100 text-sm pr-9 font-mono"
                />
                <button
            ...
            <button
              type="button"
              onClick={() => setShowEditGenerator((v) => !v)}
              className="flex items-center gap-1.5 text-xs text-cyan-400 hover:text-cyan-300 transition font-medium"
            >
              <Shuffle className="w-3.5 h-3.5" />
              {showEditGenerator
            ...
            {showEditGenerator && (
              <PasswordGeneratorPanel
                value={editGeneratorValue}
                onChange={(v) => setEditGeneratorValue(v)}
                onApply={(v) => {
                  setEditPassword(v);
                  setEditGeneratorValue(v);
                }}
              />
            )}
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                ...
                className="bg-slate-950 border-slate-800 text-slate-100 text-sm resize-none min-h-[60px]"
              />
            </div>
          </div>
        ...
        <AlertDialogContent className="bg-slate-900 border-slate-800">
          <AlertDialogHeader>
            <AlertDialogTitle className="text-white">
              {t("passwords.deleteConfirmTitle", undefined, "Delete Password?")}
            </AlertDialogTitle>
            <AlertDialogDescription className="text-slate-400">
              {t(
                "passwords.deleteConfirmDesc",
                undefined,
