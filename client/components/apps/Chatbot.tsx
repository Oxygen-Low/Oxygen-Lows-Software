...
import { Bot, User, Trash2, Loader2, Plus, Send, FileCode } from "lucide-react";
...
interface Artifact { language: string; filename: string; content: string; }
...
export function ChatbotApp() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { data: modelData } = await supabase.from("user_models").select("*");
  ...
  messages.forEach(m => {
    if (m.role === "assistant" && m.content) {
      allArtifacts.push(...parseArtifacts(m.content));
    }
  });

  return (
    <div className="flex h-[700px] gap-0 bg-slate-950/30 rounded-2xl border border-slate-800 overflow-hidden">
              ...
              <div className="space-y-6 py-4">
                {messages.map((m, i) => {
                  ...
                  return (
                    <div key={i} className={cn("flex gap-4 max-w-[85%]", m.role === "user" ? "ml-auto flex-row-reverse" : "")}>
                      <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center", m.role === "user" ? "bg-cyan-600" : "bg-slate-800")}>
                ...
                {isTyping && messages[messages.length - 1]?.role !== "assistant" && (
                  <div className="flex gap-4">
                    <div className="w-8 h-8 rounded-lg bg-slate-800 flex items-center justify-center"><Loader2 className="w-4 h-4 text-cyan-400 animate-spin" /></div>
                    <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800"><span className="animate-pulse">...</span></div>
                  </div>
                )}
                {allArtifacts.length > 0 && (
                  <div className="mt-8 border-t border-slate-800 pt-4">
                    <p className="text-[10px] font-bold text-slate-500 uppercase mb-2">Chat Artifacts</p>
                    <div className="flex flex-wrap gap-2">
                      {allArtifacts.map((art, idx) => (
                        <Button key={idx} variant="ghost" size="sm" className="h-8 bg-slate-900/30 border border-slate-800/50 hover:bg-slate-800 text-[10px]" onClick={() => setActiveArtifact(art)}>
                          <FileCode className="w-3 h-3 mr-2 text-cyan-400" />
                          {art.filename}
                        </Button>
                      ))}
                    </div>
