...
describe("RepositoriesApp", () => {
  ...
  it("handles navigation and nested paths", async () => {
    const mockRepo = { id: "repo-1", name: "test-repo", profiles: { username: "user" }, zip_size_bytes: 0, is_loaded: false };
    const mockTree = [{ name: "folder", type: "tree" }];
    const mockNestedTree = [{ name: "file.txt", type: "blob" }];

    (global.fetch as any).mockImplementation((url: string) => {
        if (url === "/api/repos") {
            return Promise.resolve({ ok: true, json: () => Promise.resolve([mockRepo]) });
        }
        ...
        if (url.includes("path=folder")) {
            return Promise.resolve({ ok: true, json: () => Promise.resolve(mockNestedTree) });
        }
    ...
    await waitFor(() => {
        expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining("path=folder"), expect.anything());
    });
    ...
    // 5. Verify file fetch
    await waitFor(() => {
        expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining("/file?path=folder%2Ffile.txt"), expect.anything());
    });
