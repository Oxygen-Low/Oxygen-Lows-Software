...
export function SurveysApp() {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto py-4 px-2 sm:px-4 space-y-6">
      {/* Header Banner */}
      <div className="bg-gradient-to-r from-primary/15 via-card/80 to-primary/10 border border-primary/25 rounded-2xl p-6 shadow-xl backdrop-blur-md">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-3">
              <div className="p-2.5 bg-primary/15 border border-primary/30 rounded-xl text-primary shadow-inner">
          ...
          {surveys.length > 0 && (
            <div className="bg-card/80 border border-border/80 rounded-xl px-4 py-2.5 text-right flex items-center gap-3 shadow-md">
      ...
      {activeView === "list" && (
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            >
              <TabsList className="bg-card/70 border border-border p-1">
          ...
          {loading ? (
            ...
            <Card className="bg-card/40 border-border text-center py-16">
              <CardContent className="space-y-3">
                <ClipboardList className="w-12 h-12 text-muted-foreground/60 mx-auto" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredSurveys.map((survey) => {
                let icon = <ClipboardList className="w-6 h-6 text-primary" />;
                ...
                return (
                  <Card
                    key={survey.id}
                    className={`flex flex-col justify-between transition-all duration-200 border bg-card/70 backdrop-blur hover:border-primary/50 hover:shadow-lg hover:shadow-primary/5 ${
                      survey.hasSubmitted ? "border-emerald-500/30" : "border-border"
                    }`}
                  >
                    ...
                    <CardContent className="py-2">
                      <div className="text-xs text-muted-foreground flex items-center justify-between border-t border-border/60 pt-2.5">
                        <span>
                          {t("surveys.questionsCount", { count: survey.questionsCount }, `${survey.questionsCount} Questions`)}
                        </span>
                        <span>
                          {survey.recurrence === "monthly"
                      ...
                      {survey.hasSubmitted ? (
                        <div className="w-full space-y-2">
                          <div className="flex items-center justify-center gap-1.5 text-xs text-emerald-400 font-medium py-1">
                            ...
                            {t("surveys.answered", undefined, "Answered this cycle")}
                          </div>
                          <Button
                          ...
                          >
                            ...
                            {t("surveys.viewResults", undefined, "View Live Results")}
                          </Button>
                        </div>
                      ) : (
                        <div className="w-full grid grid-cols-2 gap-2">
                          <Button
                            ...
                            className="w-full col-span-2 bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70 text-primary-foreground font-semibold shadow-md"
                          >
      ...
      {activeView === "taking" && activeSurvey && (
        <div className="space-y-6">
          ...
          <Card className="bg-card/80 border-border shadow-xl backdrop-blur">
            <CardHeader className="space-y-2 border-b border-border/80 pb-6">
              <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                ...
                {activeSurvey.isHardwareSurvey && (
                  <div className="bg-background/80 border border-border/80 rounded-xl p-3 flex items-center gap-3">
            ...
            <form onSubmit={handleSubmitSurvey}>
              <CardContent className="space-y-6 pt-6">
                {activeSurvey.questions.map((question, qIdx) => {
                  ...
                  return (
                    <div
                      key={question.id}
                      className="p-4 rounded-xl bg-background/50 border border-border/80 space-y-3"
                    >
                      <div className="flex items-start justify-between gap-2">
                            ...
                            return (
                              <button
                                ...
                                className={`flex items-center justify-between p-3 rounded-lg border text-sm text-left transition-all ${
                                  isSelected
                                    ? "bg-primary/15 border-primary text-primary font-semibold shadow-inner"
                      ...
                      {question.type === "number" && (
                        <Input
                          ...
                          className="bg-background border-border text-foreground placeholder:text-muted-foreground max-w-xs focus-visible:ring-primary"
                        />
                      )}
                          ...
                          {Array.from({ length: (question.max ?? 5) - (question.min ?? 1) + 1 }).map((_, i) => {
                            ...
                            return (
                              <button
                                ...
                                className={`w-11 h-11 rounded-xl font-bold text-base border transition-all ${
                                  isSelected
                      ...
                      {question.type === "multiple_choice" && question.options && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2.5 pt-1">
                          {question.options.map((option) => {
                            ...
                            const isSelected = currentArr.includes(option.value);
                              ...
                              <button
                                ...
                                onClick={() => {
                                  if (isSelected) {
                                    setAnswers((prev) => ({
                                      ...prev,
                                      [question.id]: currentArr.filter((v) => v !== option.value),
                                    }));
                                  } else {
                                    setAnswers((prev) => ({
                                      ...prev,
                                      [question.id]: [...currentArr, option.value],
                                    }));
                                  }
                                }}
                                className={`flex items-center justify-between p-3 rounded-lg border text-sm text-left transition-all ${
              ...
              <CardFooter className="border-t border-border/80 flex flex-col sm:flex-row justify-between items-center gap-4 pt-6">
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="bg-card/90 border border-border rounded-lg p-1 flex items-center gap-1">
                <Button
                  variant="ghost"
                  size="sm"
          ...
          <Card className="bg-card/80 border-border shadow-xl backdrop-blur p-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                <h2 className="text-2xl font-extrabold text-foreground">{resultsData.title} Results</h2>
                <p className="text-muted-foreground text-sm mt-0.5">
              ...
              <div className="flex items-center gap-4">
                <div className="bg-background/80 border border-border rounded-xl px-4 py-2 text-center">
                  <div className="text-xs text-muted-foreground font-semibold uppercase">{t("surveys.totalResponses", undefined, "Total Responses")}</div>
                  <div className="text-xl font-extrabold text-primary">{resultsData.totalSubmissions}</div>
                </div>
                {(resultsData.isHardwareSurvey || resultsData.surveyId === "monthly-hardware-survey") && (
                  <>
                    <div className="bg-background/80 border border-border rounded-xl px-4 py-2 text-center">
          ...
          <div className="space-y-6">
            {resultsData.questions.map((qResult, idx) => (
              <Card key={qResult.questionId} className="bg-card/80 border-border shadow-lg">
                ...
                <CardContent className="pt-6 space-y-6">
                  ...
                  {(qResult.monthlyTimeline?.length > 0 || qResult.optionsDistribution?.length > 0) && (
                    <div className="h-80 w-full bg-background/60 p-4 rounded-xl border border-border">
                        ...
                        {chartType === "line" && qResult.monthlyTimeline?.length > 0 ? (
                            ...
                            {(qResult.seriesKeys || []).map((key, keyIdx) => {
                              ...
                              return (
                                <Line
                                  ...
                                  stroke={strokeColor}
                                  strokeWidth={2.5}
                                  dot={{ r: 3, fill: strokeColor, strokeWidth: 1, stroke: "hsl(var(--card))" }}
                                  activeDot={{ r: 6 }}
                                />
                              );
                            })}
                          ...
                          >
                            ...
                            <XAxis
                              ...
                              tick={{ fill: "hsl(var(--muted-foreground))" }}
                            />
                            <YAxis stroke="hsl(var(--muted-foreground))" fontSize={11} unit="%" tick={{ fill: "hsl(var(--muted-foreground))" }} />
                            <Tooltip
                              contentStyle={{
                                backgroundColor: "hsl(var(--card, 222 47% 11%))",
                                borderColor: "hsl(var(--border, 217 33% 17%))",
                  ...
                  <div className="space-y-3 pt-2">
                    {qResult.optionsDistribution.map((opt) => (
                      <div key={opt.name} className="space-y-1">
                        ...
                        <Progress value={opt.percentage} className="h-2 bg-background border border-border" />
                      </div>
                    ))}
                  </div>
