...
describe("entityGenerator Adversarial & Stress Test Suite", () => {
  ...
  describe("Scenario 1: Adversarial Prompts, Injections, Unicode & Extreme Sizes", () => {
    ...
    it("1.2: safely handles prompt injection payloads with system prompt overrides", async () => {
      ...
      global.fetch = vi.fn().mockImplementation((url: string, init: any) => {
        if (url === "/api/ai/agent-search") {
          return Promise.resolve({
            ok: true,
            json: () => Promise.resolve({ result: "Safe search findings" }),
          });
        }
        if (url === "/api/ai/proxy") {
                ...
                choices: [
                  {
                    message: {
                      content: JSON.stringify({
                        ...
                        display_name: "Shadow",
                        short_description:
                          "A resilient character created despite injection.",
                        appearance: "Cloaked in digital camouflage.",
                        personality: "Vigilant, disciplined.",
                        backstory:
                          "Programmed to resist unauthorized system overrides.",
      ...
      expect(capturedPayload.messages[0].content).toContain(
        "OUTPUT FORMAT: You MUST respond ONLY with a valid JSON",
      );
    });
    ...
    it("1.3: safely handles rich HTML, script tags, CSS expressions, and markdown in prompt", async () => {
      ...
      global.fetch = vi.fn().mockImplementation((url: string) => {
        if (url === "/api/ai/agent-search")
          return Promise.resolve({
            ok: true,
            json: () => Promise.resolve({ result: "Lore" }),
          });
        return Promise.resolve({
          ok: true,
          json: () =>
    ...
    it("1.4: handles multi-language Unicode, emoji sequences, RTL, math symbols, and Zalgo text", async () => {
      ...
      global.fetch = vi.fn().mockImplementation((url: string) => {
        if (url === "/api/ai/agent-search")
          return Promise.resolve({
            ok: true,
            json: () => Promise.resolve({ result: "Unicode lore" }),
          });
        return Promise.resolve({
          ok: true,
          json: () =>
              ...
              choices: [
                {
                  message: {
                    content: JSON.stringify({
                      ...
                      short_description:
                        "Master of multilingual runes and glyphs.",
                      appearance:
                        "Robes etched with ℵ₀ and glowing math fractals.",
                      personality: "Stoic.",
    ...
    it("1.5: handles massive prompt payload (100,000+ characters) without stack overflow or memory crash", async () => {
      const massivePrompt =
        "A futuristic knight defending the celestial citadel. ".repeat(2000); // ~108,000 chars
  ...
  describe("Scenario 2: Malformed, Truncated & Conversational LLM Outputs", () => {
    ...
    it("2.4: throws clear error when generator returns empty string, non-string, or purely non-JSON text", () => {
      expect(() => extractJsonPayload("")).toThrow(
        "Empty response received from generator",
      );
      expect(() => extractJsonPayload(null as any)).toThrow(
        "Empty response received from generator",
      );
      expect(() => extractJsonPayload(undefined as any)).toThrow(
        "Empty response received from generator",
      );
      expect(() =>
        extractJsonPayload("I cannot fulfill this request as an AI assistant."),
      ).toThrow("Failed to parse structured JSON from generator output");
    });
                ...
                choices: [
                  {
                    message: {
                      content: JSON.stringify({
                        ...
                        short_description:
                          "Generated while offline from search agent.",
                      }),
                    },
                  },
                ],
  ...
  describe("Scenario 5: Local Model Host Switching (127.0.0.1 <-> localhost)", () => {
              ...
              Promise.resolve({
                message: {
                  content: JSON.stringify({
                    ...
                    display_name: "Ollama",
                    short_description:
                      "Successfully retrieved from localhost fallback.",
                  }),
                },
              }),
          });
    ...
    it("5.2: automatically retries localhost when 127.0.0.1 fails for local LM Studio", async () => {
                ...
                choices: [
                  {
                    message: {
                      content: JSON.stringify({
                        ...
                        display_name: "LM",
                        short_description:
                          "Generated on LM Studio via localhost.",
                      }),
                    },
                  },
                ],
      ...
      expect(attemptedUrls).toContain(
                ...
                choices: [
                  {
                    message: {
                      content: JSON.stringify({
                        ...
                        display_name: "Kobold",
                        short_description:
                          "Generated on KoboldCPP via localhost.",
                      }),
                    },
                  },
                ],
    ...
    it("5.4: propagates error when both 127.0.0.1 and localhost endpoints fail", async () => {
      ...
      await expect(
        ...
        "Connection refused on http://localhost:11434/api/chat",
      );
    });
  });
