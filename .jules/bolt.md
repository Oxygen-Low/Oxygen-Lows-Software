...
## 2024-07-19 - [Avoid O(N) array operations in render for streaming components]

**Learning:** In frequently re-rendering components like streaming React interfaces (e.g., `Chatbot.tsx`), running array filtering or mapping operations (like `.filter` or `.some`) directly in the render body causes extremely expensive O(N) recalculations on every keystroke or streaming token.
**Action:** Memoize derived arrays using `useMemo` and evaluate conditions early outside the render block.
