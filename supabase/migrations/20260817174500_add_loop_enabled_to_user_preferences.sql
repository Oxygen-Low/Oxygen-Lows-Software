-- Add loop_enabled column to user_preferences
ALTER TABLE public.user_preferences ADD COLUMN IF NOT EXISTS loop_enabled BOOLEAN DEFAULT false;

-- Redefine upsert_user_preferences with p_loop_enabled
DO $$
DECLARE
    r RECORD;
BEGIN
...
BEGIN
  ...
  INSERT INTO public.user_preferences (
    ...
    shuffle_enabled, use_gradient, last_model_id, last_provider, custom_models, profile_picture_path, loop_enabled, updated_at
  )
  VALUES (
    auth.uid(),
  ...
  ON CONFLICT (user_id) DO UPDATE SET
    ...
    loop_enabled = COALESCE(p_loop_enabled, public.user_preferences.loop_enabled),
    updated_at = now();
END;
